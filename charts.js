...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var sampleValue = samplesFiltered[0];
    ...
    top10_otu_ids = otu_ids.slice(0,10)
    ...
    metadataArrayFiltered = metadataArray.filter(sampleObj => sampleObj.id == sample);

    // Create a variable that holds the first sample in the array.
    // 2. Create a variable that holds the first sample in the metadata array.
    var resultMetadata = metadataArrayFiltered[0];
    console.log(resultMetadata);

    // 3. Create a variable that holds the washing frequency.
    var washFreq = resultMetadata.wfreq.toFixed(1);
    ...
    var layout = { width: 480, height: 400};
